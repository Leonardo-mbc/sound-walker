export const container: string;
export const confirmShow: string;
export const arrivalItemsShow: string;
export const arrivalItemsHide: string;
export const arrivalMessageShow: string;
export const arrivalMessageHide: string;
export const shareContainerShow: string;
export const audioEnabler: string;
export const show: string;
export const title: string;
export const playButton: string;
export const locked: string;
export const shareButton: string;
export const backButton: string;
export const discList: string;
export const confirmCover: string;
export const confirm: string;
export const answerSet: string;
export const discTitle: string;
export const answer: string;
export const play: string;
export const arrivalContainer: string;
export const arrivalItems: string;
export const hide: string;
export const arrivalMessage: string;
export const unlockedText: string;
export const confirmText: string;
export const tutorialContainer: string;
export const shareContainer: string;
export const shareBox: string;
export const shareButtons: string;
export const shareClose: string;
export const shareCloseButton: string;
